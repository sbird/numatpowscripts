...
while(<$INHAND>){
        s/^\s*output_root\s*=\s*[\w\/\.-]*/output_root=out\/nu$o_nu/i;
        #Set neutrino mass
        ...
        s/^\s*do_nonlinear\s*=\s*[\w\/.-]*/do_nonlinear = 1/i;
        ...
        #Set up output
        s/^\s*transfer_kmax\s*=\s*[\w\/.-]*/transfer_kmax = 10/i;
        s/^\s*transfer_num_redshifts\s*=\s*[\w\/.-]*/transfer_num_redshifts = $#red/i;
